...
选择源目录 → 输入密码 → 选择算法 → 开始批量加密。递归加密目录下所有文件（CLI 2.x 输出为混淆名）。若存在续传文件（`.prs`），引擎会自动从中断点继续。并发线程数由引擎的 `fileencryptor.yaml`（`worker_threads`，0=自动）配置，GUI 不再提供线程数选项。
...
> 引擎版本要求：Windows 与 Linux 均需 **官方 CLI 2.4.0**（格式 v6；线程数等运维参数由引擎 `fileencryptor.yaml` 配置；批量解密经 `-rn` 自动还原原始文件名）。
