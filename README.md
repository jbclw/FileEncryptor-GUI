...
| 平台 | 引擎文件名 | 说明 |
|---|---|---|
| Windows | `FileEncryptor.exe` | **官方 CLI 2.4.0**（原项目拆分的独立 CLI 组件） |
| Linux | `FileEncryptor` | **官方 CLI 2.4.0**（提供自包含 DEB/RPM 包，安装后通常位于 `/usr/bin/FileEncryptor`，GUI 也能从 PATH 找到） |
...
- **生成 SHA-256 校验单**：加密成功后额外输出 `<产物>.ptd.sha256` 校验文件
- **非对称加密（rage）**：勾选后改为用**收件人公钥**加密（填 `age1…` 字符串或公钥文件），不需要密码，产物为 `.age` 文件；点「生成密钥对」可现场生成一对 X25519 密钥（私钥存为所选目录下的 `rage_private.txt`，公钥打印在运行日志里）。该模式下 zstd 与 SHA-256 不适用（引擎会忽略）

> 也可用**密钥文件**代替密码：在"密钥文件"栏选择一个文件，其内容即作为密码（对应 CLI 的 `-k`，非交互读取），此时忽略密码输入框。

### 2. 解密单个文件

选择 `.ptd` 文件 → 输入密码 → 开始解密。解密 rage 产物（`.age`）时勾选「rage 非对称解密」，并在"密钥文件"栏选择**私钥文件**（`AGE-SECRET-KEY-…`）。

### 3. 批量加密目录

选择源目录 → 输入密码 → 选择算法 → 开始批量加密。递归加密目录下所有文件（CLI 2.x 输出为混淆名）。若存在续传文件（`.prs`），引擎会自动从中断点继续。并发线程数由引擎的 `fileencryptor.yaml`（`worker_threads`，0=自动）配置，GUI 不再提供线程数选项。同样支持"删除源文件 / 移到回收站"、zstd 压缩、SHA-256 校验单与密钥文件。（**批量模式不支持 rage 非对称加密**，引擎侧 `-be` 不接受 `-m rage`。）

### 4. 批量解密目录

选择包含 `.ptd` 文件的目录 → 输入密码 → 开始批量解密，自动还原原始文件名。
...
- **关于页**：显示 GUI 版本、**运行时探测**的引擎版本与能力（zstd / AEGIS）、引擎路径、运行环境、开源依赖、友情链接与成员主页（B 站 / GitHub），并支持「复制全部信息」便于反馈问题时粘贴

> 引擎版本要求：Windows 与 Linux 均需 **官方 CLI 2.4.0 及以上**（已验证 2.4.1；格式 v6；线程数等运维参数由引擎 `fileencryptor.yaml` 配置；批量解密经 `-rn` 自动还原原始文件名）。
...
- **单文件加密输出名为混淆名**：CLI 2.x 默认开启输出名混淆（十六进制名 + `.ptd` 扩展名），加密成功后 GUI 会在日志中提示输出位置
...
A: 检查日志区的输出信息。常见原因：密码输入错误、输出目录已存在同名文件、引擎版本不兼容（请确认使用官方 CLI 2.4.0）。
